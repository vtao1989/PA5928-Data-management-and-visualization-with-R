...
## Introduction the dataset and research question

The dataset we used here is from the General Social Survey (GSS) from the National Opinion Research Center of the University of Chicago. The dataset shows the results of the vocabulary tests for several years. It contains eight variables [@GSSvocab]. The descriptions are listed below.
...
For a complete plot, we have add more infomation such as title, x label, and y label, etc.
...
Well, some plots even do not need you to manipulate the data. You could draw the plot directly.

```{r}
ggplot(GSSvocab, aes(x = ageGroup, y = vocab)) +
  geom_boxplot() +
  facet_wrap(~year) +
